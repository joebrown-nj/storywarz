...
class WarzRoundVote extends Model
{
    /** @use HasFactory<\Database\Factories\WarzFactory> */
    use HasFactory;

    // protected $table = 'warz_rounds_votes';
    ...
    {
        return $query->select('warz_round_votes.*')
            ...
            ->addSelect(DB::raw('(SELECT score from warz_round_scores where warz_round_scores.warz_rounds_id = warz_round_votes.warz_rounds_id and warz_round_scores.user_id = warz_round_votes.user_id) as score'))
            ->join('users as u', 'u.id', '=', 'warz_round_votes.user_id')
            ->join('users as vu', 'vu.id', '=', 'warz_round_votes.voted_for_user_id');
    }

    public function scopeForRound(Builder $query, int $warRoundId): Builder
    {
        return $query->where('warz_round_votes.warz_rounds_id', $warRoundId);
    }
    ...
    public static function findUserVoteForRound(int $userId, int $warRoundId): ?self
    {
        return static::query()
            ->withVoteDetails()
            ->where('warz_round_votes.user_id', $userId)
            ->forRound($warRoundId)
            ->first();
    }
}
